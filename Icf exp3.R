...
Indicator4 = function(i,j,Ha,Wa,p){
  x = Wa + 2*p + 1 - Wt
  y = Ha + 2*p + 1 - Ht
  if((Ht+k-1)-y < i & (Wt+k-1)-x < j) answer = 1
  else answer = 0
  return(answer)
}
...
set.seed(1)
w = matrix(runif((Wt+k+1)*(Ht+k+1)), nrow=(Ht+k+1), ncol=(Wt+k+1)) ## 4개의 채널을 만들기 위해서는 max(Wt)+k에다가 1번씩 더 있어야한다.
print(w)
...
Iconv1 = matrix(v1, nrow=(Ht+k-1), ncol=(Wt+k-1))
Iconv2 = matrix(v2, nrow=(Ht+k-1), ncol=(Wt+k-1))
Iconv3 = matrix(v3, nrow=(Ht+k-1), ncol=(Wt+k-1))
Iconv4 = matrix(v4, nrow=(Ht+k-1), ncol=(Wt+k-1))
...
add_m1 = matrix(0, nrow=nrow(zero_m), ncol=((2*Wt+k-2)-ncol(zero_m)))
add_m2 = matrix(0, nrow=((2*Ht+k-2)-nrow(zero_m)), ncol=ncol(zero_m))
add_m3 = matrix(0, nrow=((2*Ht+k-2)-nrow(zero_m)), ncol=((2*Wt+k-2)-ncol(zero_m)))
...
conv_sum = function(target_m, filter_m, Iconv_col, Iconv_row){
  tmp = 0
  result = 0
  for(j in 1:Iconv_row){
    for(i in 1:Iconv_col){
...
conv_operate = function(zero_m_a, Iconv, Ht, Wt){
  ...
  for(j in 1:Wt){
    for(i in 1:Ht){
      value = c(value, conv_sum(zero_m_a[i:(i-1+nrow(Iconv)),j:(j-1+ncol(Iconv))], Iconv, ncol(Iconv), nrow(Iconv)))
    }
  }
