...
pair_of_MWI =  3 # input Width의 최댓값의 순서쌍 중 최솟값 but test data를 고려하여 1로 하는 것이 좋다

pair_of_MHI = 3 # input Height의 최댓값의 순서쌍 중 최솟값 but test data를 고려하여 1로 하는 것이 좋다
...
ReLU_ch1 = sapply(ch1, ReLU)
ReLU_ch2 = sapply(ch2, ReLU)
ReLU_ch3 = sapply(ch3, ReLU)
ReLU_ch4 = sapply(ch4, ReLU)

#### Reshaping for fully connected layer ####
fcl = c(ReLU_ch1, ReLU_ch2, ReLU_ch3, ReLU_ch4)

#### Fully connected layer ####
set.seed(3)
w2 = runif(nrow(ch1)*ncol(ch2)*4*10, min=-1, max=1)

fcl2 = c(1.7800, -0.7537,  0.5405, -2.5528, -0.6646, -0.5985, -0.9551,  0.8287,
         -0.0529, -0.5071)
for(mlp in 1:10){
  fcl2[mlp] = sum(fcl*w2[((mlp-1)*nrow(ch1)*ncol(ch2)*4+1):(mlp*nrow(ch1)*ncol(ch2)*4)])
}
...
for(k in 1:36){
  print(w2[k]*dev_relu(c(ch)[k])*c(zero_m_a1)[k]*c(Iconv1[k]))
}
